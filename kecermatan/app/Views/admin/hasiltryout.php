  ...
  <?= $this->include('front/sidebar') ?>
        ...
        <div class="flex-1 items-center p-3 text-xl bg-slate-100" >
          ...
          <div class=" flex-1 p-2 font-bold text-lg">
                <h1 class="mb-4 font-bold bg-three px-4 py-4 rounded-lg shadow-md text-two">HASIL PENILAIAN : <?= $infoPeserta[0]->person_nm ?></h1>
                <!--DATA TABLE -->
                ...
                <table class="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    // echo $used[0]->maxused;exit;
                        ...
                        foreach ($kolom as $kkolom) {
                            ...
                            $salah = 0;
                            $soal_terjawab = 0;
                            $query = $db->query("SELECT *,a.pilihan_nm AS pilihan_respon FROM respon a LEFT JOIN soal b ON b.soal_id = a.soal_id LEFT JOIN jawaban c ON c.jawaban_id = a.jawaban_id WHERE a.materi = 98 AND b.sk_group_id = ".$request->uri->getSegment(4)." AND a.created_user_id = ".$request->uri->getSegment(7)." AND a.used = ".$request->uri->getSegment(8)." AND a.kolom_id = $kkolom->kolom_id AND a.group_id = ".$request->uri->getSegment(5)." AND a.user_group = '".$request->uri->getSegment(6)."'")->getResult();
